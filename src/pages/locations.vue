      ...
      <q-container>
        ...
        <div class="q-pa-sm">
          <div class="row justify-center">
            <img src="../assets/rick-and-morty-locations.svg" style="width: 300px;" alt="rickandmorty" class="image-home">
          </div>
          <div class="row justify-center">
            <h6 class="text-deep-purple-7">LOCALIZAÇÕES</h6>
          </div>
          <div class="row justify-center">
          ...
          <div class="row justify-center mt-4">
            <div v-for="(item, index) in locations" :key="index">
              <router-link :to="`/local/${item.id}`">
                <cardDefault :id="item.id" :title="item.name" :subtitle="''"> </cardDefault>
              </router-link>
            </div>
          </div>
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function getDataList() {
      ...
      try {
        const i:any = await storeAPI.getLocations(page.value, search.value)
        locations.value = i.results
